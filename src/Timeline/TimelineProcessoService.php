...
class TimelineProcessoService
{
    ...
    protected const POPULATE_USUARIO = [
        'imgPerfil',
    ];
     ...
     */
    public function __construct(private MapperManager $mapperManager,
                                private ArrayQueryBuilder $arrayQueryBuilder){
    }
    ...
    public function getTimelineProcesso(Processo $processo, array $criteria): array
    {
        ...
        $qb = $this->arrayQueryBuilder
            ->buildQueryBuilder([
                'object' => Tarefa::class,
                'filter' => $criteria,
                'sort' => ['dataHoraConclusaoPrazo' => 'ASC', 'id' => 'ASC'],
            ])
            ->setMaxResults(null);
        ...
        foreach ($tarefas as $tarefa) {
            ...
            uasort(
                $eventsTarefa,
                fn($eventA, $eventB) =>
                    $eventA->getEventDate()->getTimestamp() - $eventB->getEventDate()->getTimestamp()
            );
    ...
    /**
     * @param EntityInterface $tarefa
     * @param array $events
     * @return void
     * @throws ReflectionException
     */
    protected function processTarefaDistribuicoesEvents(
        EntityInterface $tarefa,
        array &$events
    ): void
    {
        ...
        foreach ($tarefa->getDistribuicoes() as $distribuicao) {
            ...
            if (!$distribuicao->getUsuarioAnterior()) {
                $events[] = (new TimelineEvent())
                    ->setMessage(sprintf(
                        ...
                        $this->formataNomeUsuario($distribuicao->getUsuarioPosterior(), $distribuicao->getSetorPosterior())
                    ))
            ...
            if ($distribuicao->getUsuarioAnterior()) {
                $events[] = (new TimelineEvent())
                    ->setMessage(sprintf(
                        ...
                        $tarefa->getId(),
                        $this->formataNomeUsuario($distribuicao->getUsuarioAnterior(), $distribuicao->getSetorAnterior()),
                        $this->formataNomeUsuario($distribuicao->getUsuarioPosterior(), $distribuicao->getSetorPosterior())
                    ))
                    ->setEventDate($distribuicao->getDataHoraDistribuicao())
                    ->setUsuario($usuarioDTO)
                    ...
                    ->setObjectContext(json_encode([
                        'usuarioAnterior' => $this->formataNomeUsuario($distribuicao->getUsuarioAnterior(), $distribuicao->getSetorAnterior()),
                        'usuarioPosterior' => $this->formataNomeUsuario($distribuicao->getUsuarioPosterior(), $distribuicao->getSetorPosterior()),
                    ]))
                    ->setTarefa($tarefaDTO);
            }
        }
     ...
     * @throws ReflectionException
     */
    protected function processTarefaDocumentosAvulsoEvents(
        EntityInterface $tarefa,
        array &$events
    ): void
    {
            ...
            if ($documento->getDataHoraResposta()) {
                $events[] = (new TimelineEvent())
                    ->setMessage(sprintf(
                        ...
                        $documento->getEspecieDocumentoAvulso()->getNome(),
                        $this->formataNomeUsuario($documento->getUsuarioResposta(), $documento->getSetorResponsavel())
                    ))
