...
class JuntadaRepository extends BaseRepository
{
     ...
     * @param array<int> $juntadasId
     * @return array
     */
    public function getJuntadasProcessoSize(int $processoId, array $juntadasId = []): array
    {
        $qb = $this->getEntityManager()->createQueryBuilder();

        $qb
            ->select('SUM(cd.tamanho) as tamanho_juntada', 'j.id')
        ...
        if (!empty($juntadasId)) {
            $qb->andWhere(
                $qb->expr()->in('j.id', ':juntadasId')
            );
            $qb->setParameter('juntadasId', $juntadasId);
        }
