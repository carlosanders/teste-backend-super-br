 ...
 * @author Advocacia-Geral da União <supp@agu.gov.br>
 */
class LoadAssinaturaData extends Fixture implements OrderedFixtureInterface, ContainerAwareInterface, FixtureGroupInterface
{
...
EOD
        ...
        $assinatura->setComponenteDigital($this->getReference('ComponenteDigital-TEMPLATE DESPACHO'));
        $assinatura->setDataHoraAssinatura(\DateTime::createFromFormat('Y-m-d', '2021-12-05'));
        $assinatura->setAlgoritmoHash('SHA256');
        $assinatura->setCriadoPor($this->getReference('Usuario-00000000002'));

        // Persist entity
        $this->manager->persist($assinatura);
...
EOD
        ...
        $assinatura->setComponenteDigital($this->getReference('ComponenteDigital-MODELO DESPACHO EM BRANCO'));
