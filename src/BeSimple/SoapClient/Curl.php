    ...
    public function __construct(array $options = [], $followLocationMaxRedirects = 10)
    {
        ...
        if (isset($options['proxy_host'])) {
            if (false !== $options['proxy_host']) {
                $proxyHost = $options['proxy_host'].(isset($options['proxy_port']) ? $options['proxy_port'] : 8_080);
            } else {
                $proxyHost = false;
            }
        ...
        if (isset($options['login'])) {
            curl_setopt(
                $this->ch,
                CURLOPT_HTTPAUTH,
                isset($options['extra_options']['http_auth']) ? $options['extra_options']['http_auth'] : CURLAUTH_ANY
            );
