...
class LoginUnicoGovBrService
{
     ...
     * @return array
     */
    public function getAuthorizationData(string $code): array
    {
     ...
     * @throws Exception
     */
    public function retornaDadosUsuario(string $code): ?array
    {
        $authData = $this->getAuthorizationData($code);
    ...
    /**
     * Valida as confiabilidades.
     *
     * @param array $confiabilidadesUsuario
     *
     * @return bool
     */
    public function validaConfiabilidadesUsuario(array $confiabilidadesUsuario): bool
    {
        $confiabilidadesValidas = $this->getConfiabilidadesValidas();

        foreach ($confiabilidadesUsuario as $confiabilidade) {
            if (isset($confiabilidadesValidas[$confiabilidade['categoria']['id']])
                && $confiabilidadesValidas[$confiabilidade['categoria']['id']] == $confiabilidade['categoria']['nivel']
            ) {
                return true;
            }
        }
