...
class BarramentoSincronizacaoDocumento
{
    ...
    public function sincronizaDocumentoAvulso(
        ...
        try {
            //Caso não exista protocolo relacionado, cria o processo
            if (!isset($this->response->metadados->documento->protocolo)) {
                $processoEntity = $this->criaProcesso($metadadosDocumento, $pessoaRemetente, $nre);
            } else {
                ...
                //#TO DO - Transformar em uma function
                /*
                ...
                //Retorna dados do documento avulso
    ...
    private function criaProcesso(
        ...
        $processoDto->setDataHoraAbertura(
            DateTime::createFromFormat('Y-m-d\TH:i:sP', $metadadosDocumento->dataHoraDeProducao)
        );
        ...
        $processoDto->setSetorAtual($this->setorDestinatario);

        // Cria NUP
        $numeroUnicoProtocolo = $this->numeroUnicoProtocoloResource->generate($this->setorDestinatario);
        $processoDto->setNUP($numeroUnicoProtocolo);
