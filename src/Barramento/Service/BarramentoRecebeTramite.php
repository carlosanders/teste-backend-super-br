...
class BarramentoRecebeTramite extends AbstractBarramentoManager
{
    ...
    public function receberTramite(int $idt, string $transactionId)
    {
        ...
        // Caso já exista StatusBarramento para esse IDT, o tramite já foi passou da fase 2
        // Verificação para os casos que a fila duplicou para processamento do status 2
        ...
        $this->logger->info(microtime(true) - $tempoExecucao, ['status']);
    ...
    private function repositorioToPessoa(
        ...
        if (!$vinculacaoPessoaEntity) {
            $hierarquia_nome = '';
            if ($consultarEstruturasResponse->estruturasEncontradas->estrutura->hierarquia &&
                $consultarEstruturasResponse->estruturasEncontradas->estrutura->hierarquia->nivel) {
