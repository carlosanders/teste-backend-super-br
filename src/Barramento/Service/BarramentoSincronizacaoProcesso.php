...
class BarramentoSincronizacaoProcesso
{
    ...
    public function sincronizaProcesso(
        ...
        if (!$processoEntity) {
            ...
            // caso processo existente sem tramitacao, é um tramite de processo cadastrado manualmente
            if (!$tramitacaoBarramento) {
                // atualiza origem dados do processo
        ...
        try {
            ...
            throw new Exception('Erro ao salvar a tramitação.');
        }
        ...
        try {
            ...
            $this->logger->critical("{$e->getMessage()}.' - '.{$e->getTraceAsString()}");
            throw new Exception('Erro ao sincronizar documentos.');
        }
        ...
        try {
            ...
            $this->logger->critical("{$e->getMessage()}.' - '.{$e->getTraceAsString()}");
            throw new Exception('Erro no cadastro de interessados. Verifique os campos dos interessados.');
        }
