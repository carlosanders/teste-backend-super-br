...
run-tests: ## Runs all tests via phpunit (Uses PCOV if that is installed)
	...
	@XDEBUG_MODE=off ./vendor/bin/phpunit --bootstrap "./tests/bootstrap.php" --configuration "phpunit.fastest.xml" --coverage-html build/logs/coverage --log-junit build/logs/junit.xml --verbose
